...
typedef struct militar
{
  ...
  int numreg;    // nmero do registro da carreira
} militar;
...
typedef struct carreira
{
  ...
  char org[20];     // organizao - exemplo: exrcito, aeronutica, marinha
  char patente[20]; // exemplo: soldado, cabo, sargento
  float salario;    // valor do salrio
} carreira;
...
main()
{
  ...
  do
  {
    ...
    {
      case 1:
        aloca_militar(&pmil, cont + 1);
        cadastra_militar(pmil + cont, pcar);
        cont++;
        break;
      case 2:
        troca_patente(pmil, cont, pcar);
        break;
      case 3:
        mostra_patente(pcar, 5);
        break;
    }
  } while (op != 4);
}
...
void troca_patente(militar *pm, int qm, carreira *pc)
{
  ...
  {
    printf("\nOrganizacao Atual: %s\nPatente Atual: %s\nSalario Atual: %.2f\n\n", (pc + n_reg - 1)->org, (pc + n_reg - 1)->patente, (pc + n_reg - 1)->salario);
    (pc + n_reg - 1)->qmilitar--;
    printf("\nEscolha a NOVA Organizao: Exercito / Aeronautica / Marinha\n");
    gets(aux_org);
    fflush(stdin);
    maiuscula(aux_org);
    printf("\nDigite a patente: ");

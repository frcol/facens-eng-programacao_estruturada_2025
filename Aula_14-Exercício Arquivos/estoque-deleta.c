...
void cadastro(dados *p)
{
	...
	if (ind == -1)		   // nao ha vago
		...
		grava(p, "rb+", ind); // sobreescrever no lugar do vago
...
int busca(dados *p, int num_reg)
{
	...
	else
	{
		for (i = 0; i < qreg; i++)
		{
			...
			if (p->reg == num_reg) // achou registro      && p->qtde != -1
			{
				achou = i; // posicao do registro
				i = qreg;  // forca a saida do for
			}
			else if (p->reg == num_reg && p->qtde == -1) // achou registro vago
			{
				printf("\nRegistro VAGO\n");
				achou = -1; // nao achou registro
				i = qreg;   // forca a saida do for
			}
			else if (p->reg != num_reg && i == qreg - 1) // nao achou registro
			{
				achou = i;
				i = qreg; // forca a saida do for
			}
